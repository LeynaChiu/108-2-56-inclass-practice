...
" 延長工作時間超過法令規定"-> mostViolatedArticle_content
...
```{r ans15}
table(badEmployers$dt)->popularSentenceMonth
...
```{r ans22}
stringr::str_detect(students10$`志願`,"北大")->ntpu_namelist
students10$`姓名`[ntpu_namelist]->ntpu_namelist
ntpu_namelist
...
logical儲存的本質是甚麼?
```{r ans23}
ntpu_econtpu <-students10$`志願`=="北大經濟"
pick_12 <-students10$`國文`[ntpu_econtpu]>=12
pick_english<-students10$`英文`[ntpu_econtpu]
pick_math<-students10$`數學`[ntpu_econtpu]
pick_26<-pick_english+pick_math>=26
pick_ntpu<-students10$`姓名`[ntpu_econtpu]
pick1<-pick_ntpu[pick_12]
ntpu_econtpu_eco<-pick1[pick_26]
ntpu_econtpu_eco
```
...
```{r ans24}
congrats<-paste0(c("恭喜"),c(ntpu_econtpu_eco),
                 c("同學錄取國立臺北大學經濟系"))
congrats
```
