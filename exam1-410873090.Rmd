...
```{r ans1}
factor(numerals_Gujarati)->numerals_Gujarati
as.integer(numerals_Gujarati)
numerals_Gujarati
factor(receipts300)->receipts300
sum(as.integer(receipts300))->sum300
sum300
```
...
```{r ans32}
factor(post$comments$gender)->post$comments$gender
...
```{r ans33}
library(lubridate)
post$category_dateTime$DateTime <- ymd_hm("2020-04-14 14:44",tz="Asia/Taipei")
post
```
...
datetime1<-ymd_hm(c("2020-04-25 15:50"),tz="Europe/Warsaw")
datetime1<with_tz(datetime1,tzone = "Asia/Taipei")
print(datetime1)
post$comments$DateTime[[26]]<-"4月25日 21:50"
...
```{r ans35}
post[[4]]<-"category"
post[[4]]<-list(`category`="台北大學",`dateTime`="2020-04-20 14:44:00")
