...
單號請用1表示、雙號則用0表示。
```{r ans21}
last_number<-as.numeric(
  str_extract(Mask$"身分證字號"[[1]],"[0-9]$"))

`1st_odd_or_even`<- if (last_number %% 2 == 0){
  `1st_odd_or_even`= 0
}else{
  `1st_odd_or_even`= 1
}
...
next_date <-
  function(n){
    if (wday(today()) %in% chart[[odd_or_even(n)+1]]){
  date <- paste(today(),weekdays(as.Date(today())),sep=',')
}else{
  date <- paste(today()+1,weekdays(as.Date(today()+1)),sep=',')
}
    return(date)
  }
...
```{r ans31}
class(names(table(courseStructure$`科目名稱`[courseStructure$`學年` == 100 &
                                                str_extract(courseStructure$`應修系級`,paste0("經濟系", "1")) == paste0("經濟系", "1")])))
get_courses <- function(entranceYear, major){
value <- list(
  year1=names(table(courseStructure$`科目名稱`[courseStructure$`學年` == entranceYear &
                                                str_extract(courseStructure$`應修系級`,paste0(major, "1")) == paste0(major, "1")])),
  year2=names(table(courseStructure$`科目名稱`[courseStructure$`學年` == entranceYear+1 &
                                                str_extract(courseStructure$`應修系級`,paste0(major, "2")) == paste0(major, "2")])),
  year3=names(table(courseStructure$`科目名稱`[courseStructure$`學年` == entranceYear+2 &
                                                str_extract(courseStructure$`應修系級`,paste0(major, "3")) == paste0(major, "3")])),
  year4=names(table(courseStructure$`科目名稱`[courseStructure$`學年` == entranceYear+3 &
                                                str_extract(courseStructure$`應修系級`,paste0(major, "4")) == paste0(major, "4")]))
   )
 return(value)
  }
get_courses(100,"經濟系")
...
list_econCourseCounts <-  list(
  entranceYear=100:103,
  year1 = c(length(get_courses(100, "經濟系")[[1]]), length(get_courses(101, "經濟系")[[1]]), length(get_courses(102, "經濟系")[[1]]), length(get_courses(103, "經濟系")[[1]])),
  year2 = c(length(get_courses(100, "經濟系")[[2]]), length(get_courses(101, "經濟系")[[2]]), length(get_courses(102, "經濟系")[[2]]), length(get_courses(103, "經濟系")[[2]])),
  year3 = c(length(get_courses(100, "經濟系")[[3]]), length(get_courses(101, "經濟系")[[3]]), length(get_courses(102, "經濟系")[[3]]), length(get_courses(103, "經濟系")[[3]])),
  year4 = c(length(get_courses(100, "經濟系")[[4]]), length(get_courses(101, "經濟系")[[4]]), length(get_courses(102, "經濟系")[[4]]), length(get_courses(103, "經濟系")[[4]]))
  )
